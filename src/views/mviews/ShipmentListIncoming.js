...
let ShipmentListIncoming = React.createClass({
  propTypes:{
  ...
  render:function(){
    ...
    if (this.state.data.length === 0){
      content = [
        (<View style={styles.container} key="view">
          <Text>Watch your incoming shipments</Text>
        </View>)
      ];
    } else {
      content = [
    ...
    content.push(<TouchableHighlight key="button" onPress={() => {}} style={styles.button}
      ...
      <View style={styles.container}>
        <Text style={styles.buttontext}>Add from Code</Text>
      </View>
    </TouchableHighlight>);
  ...
  load:function(){
    this.props.apiRequester('GET','/self/shipments/incoming').then((response)=>{
      if (!response.ok){
...
module.exports = ShipmentListIncoming;
